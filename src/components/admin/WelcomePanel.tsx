...
import { useWelcomeSettings, useUpdateWelcomeSettings, type WelcomeSettings } from "@/hooks/useWelcomeSettings";
...
import { Sparkles, Upload, Save, Eye, X as XIcon } from "lucide-react";
...
const WelcomePanel = () => {
  ...
  if (!settings) return <p className="text-xs text-muted-foreground">Carregando...</p>;
  ...
  const onUploadBg = async (file: File) => {
    ...
    try {
      ...
      const { error } = await supabase.storage.from("welcome-bg").upload(path, file, { upsert: true });
    ...
    <>
      {previewing && (
        <div className="fixed inset-0 z-[1100]">
          ...
          {/* Renderiza um overlay temporário com config atual em memória — força mostrar */}
          <PreviewOverlay form={{ ...settings, ...form } as WelcomeSettings} onClose={() => setPreviewing(false)} />
        </div>
      )}
        ...
        <div className="p-5 space-y-4">
          <div className="flex items-center justify-between p-3 rounded-xl bg-secondary/40 border border-border/40">
            <div>
              ...
              <p className="text-[11px] text-muted-foreground">Quando ligada, é exibida antes do site carregar.</p>
            </div>
          ...
          <div className="flex items-center justify-between p-3 rounded-xl bg-secondary/40 border border-border/40">
            <div>
              <p className="text-sm font-semibold">Mostrar uma vez por sessão</p>
              <p className="text-[11px] text-muted-foreground">Se desligado, aparece a cada visita.</p>
            </div>
            <Switch
              checked={!!form.show_once_per_session}
              onCheckedChange={(v) => setForm((f) => ({ ...f, show_once_per_session: v }))}
            />
          </div>
            ...
            <div className="flex items-center gap-3 mt-1">
              {form.background_image_url ? (
                <div className="relative w-32 h-20 rounded-lg overflow-hidden border border-border">
                  ...
                  <button
                    onClick={() => setForm((f) => ({ ...f, background_image_url: null }))}
              ...
              <label className="cursor-pointer">
                ...
                <Button size="sm" variant="outline" disabled={uploading} asChild>
                  <span><Upload className="h-3.5 w-3.5 mr-1" />{uploading ? "Enviando..." : "Enviar imagem"}</span>
                </Button>
              </label>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2 pt-2">
            <Button onClick={save} disabled={update.isPending} className="bg-warning text-warning-foreground hover:bg-warning/90">
...
// Preview que ignora sessionStorage e usa o form atual
const PreviewOverlay = ({ form, onClose }: { form: WelcomeSettings; onClose: () => void }) => {
  // Reusa o WelcomeOverlay forçando enabled true e clearing session
  useEffect(() => {
    try { sessionStorage.removeItem("rubin_welcome_seen_v1"); } catch {}
  }, []);
  return (
    <div className="absolute inset-0">
      {/* Renderiza com props simuladas via override — para simplicidade, instanciamos uma cópia visual */}
      <InlineSplash form={form} onClose={onClose} />
    </div>
  );
};

const InlineSplash = ({ form, onClose }: { form: WelcomeSettings; onClose: () => void }) => {
  ...
  return (
    ...
    >
      {form.background_image_url && (
        <div aria-hidden className="absolute inset-0 opacity-20 bg-cover bg-center" style={{ backgroundImage: `url(${form.background_image_url})` }} />
      )}
      <div className="relative max-w-2xl w-full text-center">
        <h1 className="font-pixel text-2xl sm:text-4xl mb-4" style={{ color: accent }}>{form.title}</h1>
        <p className="text-sm sm:text-base text-muted-foreground mb-6">{form.subtitle}</p>
        <button
          onClick={onClose}
          className="px-8 py-4 rounded-2xl text-white font-bold"
          style={{ background: `linear-gradient(135deg, ${accent}, hsl(var(--primary)))` }}
        >
          {form.cta_text}
        </button>
      </div>
